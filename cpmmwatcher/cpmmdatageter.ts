...
import Client, { SubscribeRequestFilterTransactions } from "@triton-one/yellowstone-grpc";
import { PublicKey } from "@solana/web3.js";
import { Pubkey } from "@triton-one/yellowstone-grpc/dist/encoding/yellowstone_grpc_solana_encoding_wasm";
import base58 from "bs58";
import Decimal from "decimal.js";
//import { grpcToken, grpcUrl } from "../config";
import Redis from "ioredis";

const redis = new Redis({ host: "localhost", port: 6379 }); // connect to the KeyDB service
...
async function cpmmPoolInfo() {
  ...
  const client = new Client("http://127.0.0.1:10000"," ", undefined);
...
async function callback(_data: any, programId: string) {
  if (_data.filters.includes('ammUpdate')) {
    ...
    console.log(poolInfoCache[_poolType.poolId].vaultA?.mint.toString()+"before check");
    const mintString = poolInfoCache[_poolType.poolId].vaultA?.mint.toString();

    if (mintString && mintString.length >= 3 && mintString.substring(0, 3) !== "So1") {
      return;
    }
    console.log("sol");
    ...
    console.log(_poolType.poolId, vaultA, vaultB, vaultB.div(vaultA),poolInfoCache[_poolType.poolId].vaultB!.mint.toString())
    await redis.hset(_poolType.poolId, {
        vaultA: vaultA.toString(),
        vaultB: vaultB.toString(),
        price: vaultB.div(vaultA).toString(),
        NotSolmint: poolInfoCache[_poolType.poolId].vaultB!.mint.toString()
      });
  }
}
